    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <div className="bg-white p-8 rounded-xl shadow-md w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-2">Enter the activation code sent to your email</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Email Address</label>
            <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-yellow-500 outline-none" placeholder="student@futo.edu.ng" />
          </div>
          ...
          <div>
              ...
              <div className={`grid grid-cols-12 gap-1 ${isLoading ? 'opacity-50' : ''}`}>
                {code.map((data, index) => (
                  <input
                    ...
                    disabled={isLoading}
                    className="w-full h-12 border rounded-lg text-center text-lg font-bold focus:ring-2 focus:ring-yellow-500 outline-none disabled:bg-gray-100"
            ...
            <p className="text-xs text-gray-500 mt-2 text-center">Enter the 12-digit code sent to your email</p>
          </div>
